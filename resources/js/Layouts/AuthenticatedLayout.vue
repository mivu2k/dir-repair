...
<template>
    <div class="min-h-screen bg-[#fafafa]">
        ...
        <aside 
            :class="[
                'fixed top-0 left-0 z-50 h-screen w-56 transition-transform duration-300 border-r border-slate-100 bg-white text-slate-700 shadow-sm',
                isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
            ]"
        >
            <div class="h-full px-5 py-6 overflow-y-auto flex flex-col w-56 justify-between">
                <div>
                    <!-- Minimalist Logo / Brand Section -->
                    <div class="mb-8 flex items-center gap-2.5 px-1">
                        <div class="w-7 h-7 bg-red-600 rounded-lg flex items-center justify-center shadow-sm">
                            <span class="text-white text-[11px] font-black italic">M</span>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-xs font-black uppercase tracking-[0.2em] text-slate-900 leading-none">MEI</span>
                            <span class="text-[7.5px] font-bold text-slate-400 uppercase tracking-widest leading-none mt-1">TECHNICAL</span>
                        </div>
                    </div>
                ...
                <div class="pt-6 border-t border-slate-100">
                    <div class="flex items-center gap-3 px-1 py-1">
                        <div class="w-8 h-8 rounded-lg bg-red-50 flex items-center justify-center text-red-600 font-black text-xs uppercase">
                            {{ user?.name?.substring(0,2) }}
                        </div>
                        <div class="flex flex-col min-w-0">
                            <span class="text-xs font-bold text-slate-800 truncate max-w-[110px]">{{ user?.name }}</span>
                            <span class="text-[8.5px] font-bold text-slate-400 uppercase tracking-widest leading-none mt-1">{{ user?.roles?.[0]?.name || user?.role || 'Staff' }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div 
            ...
            class="fixed inset-0 bg-slate-900/10 backdrop-blur-sm z-40 animate-fade-in"
        ></div>

        <!-- Header -->
        <header :class="['fixed top-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-slate-100 transition-all duration-300 z-30 flex items-center px-4 md:px-6 justify-between shadow-sm', isSidebarOpen && !isMobile ? 'left-56' : 'left-0']">
            <button @click="toggleSidebar" class="p-1.5 hover:bg-slate-50 rounded-lg text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <div class="flex items-center gap-4 md:gap-6">
                <div class="hidden md:flex flex-col items-end">
                    <span class="text-[10px] font-black text-slate-800 uppercase tracking-widest leading-none">MEI SYSTEM</span>
                    <span class="text-[8px] font-bold text-emerald-500 uppercase tracking-wider flex items-center gap-1 mt-1">
...
<style scoped>
.nav-link {
    @apply flex items-center gap-3 px-3 py-2 text-[10.5px] font-bold text-slate-500 uppercase tracking-widest transition-all duration-200 rounded-lg hover:text-slate-900 hover:bg-slate-50/80;
}
.nav-link.active {
    @apply text-red-600 bg-red-50/50 font-black rounded-lg border-r-2 border-red-600;
}
.nav-link svg {
    @apply w-3.5 h-3.5 text-slate-400 transition-colors;
}
.nav-link.active svg {
    @apply text-red-600;
}
