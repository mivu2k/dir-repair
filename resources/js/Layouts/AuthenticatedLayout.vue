...
<template>
    <div class="min-h-screen bg-slate-50/50">
        ...
        <aside 
            :class="[
                'fixed top-0 left-0 z-50 h-screen transition-all duration-300 border-r border-slate-900 bg-slate-950 text-slate-200 shadow-xl shadow-slate-950/10',
                isSidebarOpen ? 'w-56 translate-x-0' : 'w-0 -translate-x-full md:w-0'
            ]"
        >
            <div class="h-full px-4 py-6 overflow-y-auto flex flex-col w-56 justify-between">
                <div>
                    ...
                    <div class="mb-8 flex items-center gap-3 px-2">
                        <div class="w-8 h-8 bg-gradient-to-tr from-rose-600 to-pink-500 rounded-xl flex items-center justify-center shadow-lg shadow-rose-600/35">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-[8px] font-bold text-rose-400 uppercase tracking-widest leading-none">TECHNICAL</span>
                        </div>
                    </div>
                ...
                <div class="pt-6 border-t border-slate-800/80">
                    <div class="flex items-center gap-3 px-2 py-1">
                        <div class="w-8 h-8 rounded-xl bg-gradient-to-tr from-rose-600 to-pink-500 flex items-center justify-center shadow-md shadow-rose-600/20 text-white font-black text-xs uppercase">
        ...
        <!-- Header -->
        <header :class="['fixed top-0 right-0 h-14 bg-white/85 backdrop-blur-md border-b border-slate-100 transition-all duration-300 z-30 flex items-center px-6 justify-between shadow-sm', isSidebarOpen && !isMobile ? 'left-56' : 'left-0']">
            ...
            <div class="flex items-center gap-6">
                ...
                <Link :href="route('logout')" method="post" as="button" class="text-[10px] font-black text-slate-400 hover:text-rose-600 uppercase tracking-widest transition-colors border-l border-slate-200 pl-4 h-5 flex items-center">Sign Out</Link>
            </div>
        </header>

        <!-- Main Content -->
        <main :class="['transition-all duration-300 pt-20 px-6 pb-8', isSidebarOpen && !isMobile ? 'ml-56' : 'ml-0']">
...
.nav-link.active {
    @apply text-white bg-gradient-to-r from-rose-600 to-pink-500 shadow-md shadow-rose-600/20 translate-x-1;
}
.nav-link svg {
