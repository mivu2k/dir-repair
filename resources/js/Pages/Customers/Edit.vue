...
const form = useForm({
    ...
    opt_in_whatsapp: props.customer.opt_in_whatsapp !== false,
    opt_in_sms: props.customer.opt_in_sms !== false,
});
...
watch(search, (value) => {
    router.get(route('customers.edit', props.customer.id), { search: value }, {
        preserveState: true,
        replace: true,
        preserveScroll: true,
    });
            ...
            <div class="bg-white border border-[#e1dfdd] rounded-sm overflow-hidden">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-[#f3f2f1]">
                        ...
                        >
                            <td class="px-3 py-2 text-[11px] font-bold text-slate-900">{{ c.name }}</td>
                            <td class="px-3 py-2 text-[11px] text-slate-600 font-mono">{{ c.phone }}</td>
                            <td class="px-3 py-2 text-[11px] text-slate-500">{{ c.organization || '---' }}</td>
                            <td class="px-3 py-2 text-[11px] text-slate-500 font-bold">{{ c.repair_jobs_count }}</td>
            ...
            >
                ...
                <div class="px-6 py-4 border-b border-[#e1dfdd] flex items-center justify-between bg-white select-none">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-extrabold text-[#201f1e]">Modify Customer Details</h2>
                    </div>
                    <button 
                        @click="handleClose"
                ...
                <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div class="w-full md:w-60 bg-[#f3f2f1] border-r border-[#e1dfdd] p-6 flex flex-col justify-between select-none">
                        <div class="space-y-6">
                            <!-- Stepper header -->
                            <h3 class="text-xs font-bold text-[#605e5c] uppercase tracking-wider">Modification Steps</h3>
                        ...
                        <div class="text-[10px] text-slate-500 font-semibold leading-relaxed border-t border-[#e1dfdd] pt-4">
                            Amending physical details, delivery addresses, and WhatsApp/SMS transaction preference rules.
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div v-if="currentStep === 'basics'" class="space-y-6">
                                <div>
                                    ...
                                    <p class="text-xs text-[#605e5c] leading-relaxed">Update basic details about the customer to maintain their profile records correctly.</p>
                                </div>
                            ...
                            <div v-if="currentStep === 'finish'" class="space-y-6">
                                <div>
                                    ...
                                    <p class="text-xs text-[#605e5c] leading-relaxed">Review the profile summary to ensure correctness before finalizing edits.</p>
                                </div>
