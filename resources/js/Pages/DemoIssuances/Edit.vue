...
const form = useForm({
    customer_id: props.demoIssuance.customer_id || '',
    items: Array.isArray(props.demoIssuance.items) ? JSON.parse(JSON.stringify(props.demoIssuance.items)) : [{ name: '', serial: '', accessories: '' }],
    expected_return_date: props.demoIssuance.expected_return_date || '',
    notes: props.demoIssuance.notes || '',
    reference_letter: props.demoIssuance.reference_letter || '',
    department: props.demoIssuance.department || '',
});

watch(search, (value) => {
    router.get(route('demo-issuances.edit', props.demoIssuance.id), { search: value }, {
        preserveState: true,
        replace: true,
        preserveScroll: true
    });
...
const submit = () => {
    form.put(route('demo-issuances.update', props.demoIssuance.id), {
        onSuccess: () => {
...
<template>
    <Head :title="`Edit Demo ${demoIssuance.issuance_number}`" />
    ...
    <AuthenticatedLayout>
        ...
        <!-- Edit Sliding Stepper Drawer (M365 Style) -->
        <div 
            ...
            >
                ...
                <div class="px-6 py-4 border-b border-[#e1dfdd] flex items-center justify-between bg-white select-none">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-extrabold text-[#201f1e]">Modify Demo Equipment ({{ demoIssuance.issuance_number }})</h2>
                    </div>
                    <button 
                        @click="handleClose"
                ...
                <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div class="w-full md:w-56 bg-[#f3f2f1] border-r border-[#e1dfdd] p-6 flex flex-col justify-between select-none">
                        ...
                        <div class="text-[10px] text-slate-500 font-semibold leading-relaxed border-t border-[#e1dfdd] pt-4">
                            Amending active serial nodes issued to customer trial departments under MEI trial logs.
                        </div>
                    </div>
