...
const form = useForm({
    type: props.gatePass.type || 'inward',
    person_name: props.gatePass.person_name || '',
    company_name: props.gatePass.company_name || '',
    vehicle_number: props.gatePass.vehicle_number || '',
    items: Array.isArray(props.gatePass.items) ? JSON.parse(JSON.stringify(props.gatePass.items)) : [{ description: '', qty: 1, serial: '' }],
    notes: props.gatePass.notes || '',
});

watch(search, (value) => {
    router.get(route('gate-passes.edit', props.gatePass.id), { search: value }, {
        preserveState: true,
        replace: true,
        preserveScroll: true
    });
...
const submit = () => {
    form.put(route('gate-passes.update', props.gatePass.id), {
        onSuccess: () => {
...
<template>
    <Head :title="`Modify Gate Pass ${gatePass.pass_number}`" />
            ...
            >
                ...
                <div class="px-6 py-4 border-b border-[#e1dfdd] flex items-center justify-between bg-white select-none">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-extrabold text-[#201f1e]">Modify Gate Pass ({{ gatePass.pass_number }})</h2>
                    </div>
                    <button 
                        @click="handleClose"
                ...
                <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div class="w-full md:w-56 bg-[#f3f2f1] border-r border-[#e1dfdd] p-6 flex flex-col justify-between select-none">
                        ...
                        <div class="text-[10px] text-slate-500 font-semibold leading-relaxed border-t border-[#e1dfdd] pt-4">
                            Amending active secure inventory transfers passing gate security checks under MEI logbooks.
                        </div>
                    </div>
